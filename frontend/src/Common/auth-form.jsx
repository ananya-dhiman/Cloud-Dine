  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      try {
        ...
        // If backend returns needsRegistration flag, register the user
        if (res.data?.needsRegistration === true) {
          console.log("User needs registration — creating account in database...");
          res = await axios.post(registerEndpoint, {
            idToken,
            name: email.split("@")[0],
            role,
        ...
        // If login fails (e.g., user not found), attempt registration
        if (
          err.response?.status === 302 ||
          err.response?.status === 404 ||
          err.response?.data?.redirect === "/register" ||
          err.response?.data?.needsRegistration === true
        ) {
          console.log("User not found locally — registering in database...");
          res = await axios.post(registerEndpoint, {
            idToken,
            name: email.split("@")[0],
            role,
